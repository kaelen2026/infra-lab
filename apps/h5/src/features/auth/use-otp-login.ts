...
import { describeAuthError } from "@/lib/messages";

export type Step = "phone" | "code";

export interface UseOtpLoginOptions {
  /** Called once verification succeeds. The page refreshes the session and redirects. */
...
/**
 * The auth screen's business logic, free of any markup. Returns a view-model the
 * UI renders directly — so the flow can be tested or reused without a DOM. Mirrors
 * `apps/web`'s hook so both browsers behave identically against the same API.
 */
